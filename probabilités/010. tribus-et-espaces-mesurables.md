...
C'est la notion clé pour les probabilités : elle formalise l'idée d'**information** au sens qualitatif — quels événements peut-on distinguer en observant les "sorties" de l'application ? La tribu engendrée par $f$ décrit ce qui est discernable via $f$ ; par extension, toute tribu sur $\Omega$ peut être vue comme les événements les plus fins qu'une application pourra distinguer.

*Ne pas confondre avec l'information au sens de Shannon.* Shannon quantifie *combien* de bits on apprend en observant une variable aléatoire ; cela nécessite une mesure de probabilité. Ici, la tribu décrit *quels* événements sont discernables, indépendamment de toute probabilité. L'entropie de Shannon se construit par-dessus, une fois une mesure fixée.

#### Définition

**Fonction mesurable.** Soient $(\Omega, \mathcal{A})$ et $(E, \mathcal{E})$ deux espaces mesurables. Une application $f : \Omega \to E$ est dite **mesurable** (ou $\mathcal{A}/\mathcal{E}$-mesurable) si
...
$\sigma(f)$ représente exactement **l'information que l'on obtient en observant $f$** : un sous-ensemble $A$ de $\Omega$ appartient à $\sigma(f)$ si et seulement si, connaissant la valeur de $f(\omega)$, on peut déterminer sans ambiguïté si $\omega \in A$ ou non.

On peut lire cela en termes de **discernabilité**. Chaque ensemble $A \in \sigma(f)$ est un test de discernabilité : il sépare $\Omega$ en deux groupes (ceux dans $A$, ceux hors de $A$). La tribu $\sigma(f)$ est la collection de tous les tests disponibles quand on n'observe que $f$. Deux points $\omega_1, \omega_2$ sont $\sigma(f)$-indiscernables si aucun de ces tests ne les sépare, ce qui revient à $f(\omega_1) = f(\omega_2)$. Les classes d'indiscernabilité sont exactement les fibres $f^{-1}(\{e\})$, $e \in E$.

Comparer des tribus revient alors à comparer des finesses de discernement : $\sigma(f) \subseteq \sigma(g)$ signifie que $g$ discerne plus finement que $f$ (toute question posable via $f$ l'est aussi via $g$, mais pas l'inverse). La mesurabilité $\sigma(f) \subseteq \mathcal{A}$ signifie que $\mathcal{A}$ discerne au moins aussi finement que $f$.

#### Exemple

Ω = {1,2,3,4,5,6} (un dé), f(ω) = ω mod 2 (pair ou impair), à valeurs dans E = {0,1} muni de 𝒫(E) = {∅, {0}, {1}, {0,1}}. Alors σ(f) = {∅, {1,3,5}, {2,4,6}, Ω} (préimages : f⁻¹({0}) = {2,4,6}, f⁻¹({1}) = {1,3,5}). Observer f sépare pair/impair, mais pas les faces individuelles. L'événement {1,2} ∉ σ(f) : il n'est pas déterminable à partir de la parité seule.

#### Lemme de factorisation (de Doob)

Soit $f : \Omega \to (E, \mathcal{E})$ comme ci-dessus, et soit $g : \Omega \to (\mathbb{R}, \mathcal{B}(\mathbb{R}))$. Alors g est $\sigma(f)$-mesurable si et seulement s'il existe une fonction mesurable $\varphi : (E, \mathcal{E}) \to (\mathbb{R}, \mathcal{B}(\mathbb{R}))$ telle que $g = \varphi \circ f$.
...
On dit que « $g$ est une fonction de $f$ » quand $g = \varphi \circ f$. La tribu $\sigma(f)$ formalise cette idée : elle est exactement la collection de tous les événements $A \subseteq \Omega$ dont on peut tester l'appartenance en ne connaissant que $f(\omega)$. Dire que $g$ est $\sigma(f)$-mesurable, c'est dire que $g$ ne requiert pas plus d'information que ce que $f$ fournit. En termes de discernabilité : $g$ est calculable à partir de $f$ si et seulement si $f$ discerne au moins aussi finement que $g$, c'est-à-dire $\sigma(g) \subseteq \sigma(f)$.
